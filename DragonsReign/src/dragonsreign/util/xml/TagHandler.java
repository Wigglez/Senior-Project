...
public class TagHandler {
	...
	final protected boolean startTag(String uri, String localName,
		...
		if (!mIsTagActive) {
			...
			if (mCurrentTagHandler >= 0) {
				return mSubTagHandlerList.get(mCurrentTagHandler).startTag(uri,
						localName, qName, attributes);
			}
			for (int i = 0; i < mSubTagHandlerList.size(); i++) {
				if (mSubTagHandlerList.get(i).startTag(uri, localName, qName,
						attributes)) {
	...
	final protected boolean endTag(String uri, String localName, String qName)
			throws SAXException {
		if (mIsTagActive) {
			if (mTagDepth > 0) {
				mTagDepth--;
			...
			if (mCurrentTagHandler >= 0) {
				if (mSubTagHandlerList.get(mCurrentTagHandler).endTag(uri,
						localName, qName)) {
	...
	final protected void characters(char[] ch, int start, int length)
			throws SAXException {
		if (mIsTagActive) {
			if (mCurrentTagHandler >= 0) {
				mSubTagHandlerList.get(mCurrentTagHandler).characters(ch,
						start, length);
			} else {
