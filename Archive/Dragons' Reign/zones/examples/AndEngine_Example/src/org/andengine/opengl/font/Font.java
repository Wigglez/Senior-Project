...
public class Font implements IFont {
	...
	public Font(final FontManager pFontManager, final ITexture pTexture, final Typeface pTypeface, final float pSize, final boolean pAntiAlias, final int pColorARGBPackedInt) {
		...
		//This was added for the shadow
		this.mShadowPaint = new Paint();
		this.mShadowPaint.setTypeface(pTypeface);
		float color = 0.4f;
		this.mShadowPaint.setColor(new Color(color, color, color, 0.25f).getARGBPackedInt());
		this.mShadowPaint.setTextSize(pSize);
		this.mShadowPaint.setAntiAlias(pAntiAlias);
		
//		this.mPaint.setShadowLayer(0.85f, 0.75f, 0.75f, new Color(0, 0, 0, 0.45f).getARGBPackedInt());
	...
	protected void drawLetter(final String pCharacterAsString, final float pLeft, final float pTop) {
		...
		this.mCanvas.drawText(pCharacterAsString, pLeft + Font.LETTER_TEXTURE_PADDING, pTop + Font.LETTER_TEXTURE_PADDING+1, this.mShadowPaint);
	    this.mCanvas.drawText(pCharacterAsString, pLeft + Font.LETTER_TEXTURE_PADDING+1, pTop + Font.LETTER_TEXTURE_PADDING, this.mShadowPaint);
	    this.mCanvas.drawText(pCharacterAsString, pLeft + Font.LETTER_TEXTURE_PADDING+1, pTop + Font.LETTER_TEXTURE_PADDING+1, this.mShadowPaint);
	    //This draws the text
	...
	private Letter createLetter(final char pCharacter) throws FontException {
		...
		final int letterWidth = this.mTextBounds.width() + 2;//ADDED BY EBRAHIM. Allows shadows to be drawn
		final int letterHeight = this.mTextBounds.height() + 2;//ADDED BY EBRAHIM. Allows shadows to be drawn
